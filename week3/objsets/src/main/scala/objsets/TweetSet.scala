...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def descendingByRetweet: TweetList =
    val best_tweet: Tweet = mostRetweeted
    Cons(best_tweet, remove(best_tweet).descendingByRetweet)
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(tweet => google.exists(y => tweet.text.contains(y)))
  lazy val appleTweets: TweetSet = allTweets.filter(tweet => apple.exists(y => tweet.text.contains(y)))
  /**
